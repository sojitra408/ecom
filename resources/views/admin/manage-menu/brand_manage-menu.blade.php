            ...
            <div class="card">
               <div class="card-header" id="heading-<?php echo("$menu->id")?>">
                  <h5 class="mb-0">
                    ...
                    <form action="{{route('manage-brand.menu.delete',$menu->id)}}" method="POST"
	               ...
	               @include('admin.manage-menu.brand_manageChild',['childs' => $menu->childs])
					@else
						 <div class="col-md-12">
                            @if(Auth::user()->can('menu-management'))
   ...
   <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content">
         ...
         <div class="modal-body">
            <form role="form" action="{{ route('brand.menu.edit',$menu->id) }}" method="post" enctype= multipart/form-data>
               {{ csrf_field() }}
               ...
               <div class="row">
                  <div class="col-lg-offset-3 col-lg-12">
                     ...
                     <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-sm" id="url" name="url" required="" value="{{$menu->url}}">
                     </div>
                     
                     <div class="form-group">
                        <label for="address">New Url</label>
                        <!--<input type="text" class="form-control form-control-sm" id="name" name="name" required="" value="">-->
                        
                        <select name="urlsecond" id="select_category" class="form-control form-control-sm"  required="" >

                                                <option value="">--- Select Category ---</option>
						<?php $subcates=getBrandByParentMenuId($menu->id); ?>
                                               
                                        @foreach ($subcates as $key1 => $value1)
                                        <option value="{{$value1->id}}" {{ ( $value1->id == $menu->urlsecond) ? 'selected' : '' }} >{{ $value1->brand_name }}</option>
...
<div class="modal fade" id="addModal-{{$menu->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content">
         ...
         <div class="modal-body">
            <form role="form" action="{{ route('brand.menu.save') }}" method="post" enctype= multipart/form-data>
               {{ csrf_field() }}
