...
<div id="collapse-<?php echo("$menu->id")?>" class="collapse show" data-parent="#accordion" aria-labelledby="heading-<?php echo("$menu->id")?>">
   <div class="card-body">
      <div class="row">
               ...
               <div class="card">
                  <div class="card-header" id="heading-<?php echo("$menu->id")?>-<?php echo("$child->parent_id")?>">
                     <h5 class="mb-0">
                        ...
                        <form action="{{route('manage-brand.menu.delete',$child->id)}}" method="POST"
                  ...
                  @include('admin.manage-menu.brand_manageChild',['childs' => $child->childs])
				@else
         ...
         <div class="col-md-1">
            @if(Auth::user()->can('menu-management'))
            <a class="btn btn-success btn-sm" role="button" data-toggle="modal" data-target="#addModal-{{$child->parent_id}}"><span class="fa fa-plus"></span></a>@endif
         </div>
   ...
   <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content" style="height: 92%;">
         ...
         <div class="modal-body">
            <form role="form" action="{{ route('brand.menu.edit',$child->id) }}" method="post" enctype= multipart/form-data>
               {{ csrf_field() }}
              
               <div class="row">
                  <div class="col-lg-offset-3 col-lg-12">
                     <div class="form-group">
                        <label for="address">Name1</label>
                        <input type="text" class="form-control form-control-sm" id="name" name="name" required="" value="{{$child->name}}">
                     </div>

                     <div class="form-group"><label for="id" class="col-md-3 control-label text-left">Brand</label><div class="">
                  
                                              <select name="select_category" id="select_category" class="form-control form-control-sm"  required="" >

                                                <option value="">--- Select Brand ---</option>
                  <?php $subcates=getBrandByParentMenuId($child->parent_id); ?>
                                               
                                        @foreach ($subcates as $key1 => $value1)
                                        <option value="{{$value1->id}}" {{ ( $child->select_category == $value1->id) ? 'selected' : '' }}>{{ $value1->brand_name }}</option>
...
<div class="modal fade" id="addModal-{{$child->parent_id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
      <div class="modal-content" style="height: 92%;">
         ...
         <div class="modal-body">
            <form role="form" action="{{ route('brand.menu.sub.save') }}" method="post" enctype= multipart/form-data>
               {{ csrf_field() }}
         ...
         <div class="modal-body">
            <form role="form" action="{{ route('brand.menu.sub.save') }}" method="post" enctype= multipart/form-data>
               ...
               <div class="row">
                  <div class="col-lg-offset-3 col-lg-12">
                     <div class="form-group">
                        <label for="address">Sub Menu Name1</label>
                        <input type="text" class="form-control form-control-sm" id="submenu" name="submenu" required="" value="{{old('name')}}">
                     </div>
                    <div class="form-group"><label for="id" class="col-md-3 control-label text-left">Brand</label><div class="">
                                              ...
                                              <select name="select_category" id="select_category" class="form-control form-control-sm"  required="" >
                                                ...
                                                <option value="">--- Select Brand ---</option>
						<?php $subcates=getBrandByParentId($child->id); ?>
