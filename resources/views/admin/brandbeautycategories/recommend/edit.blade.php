...
<div class="col-md-3">
@include('admin.brandbeautycategories.menu')

</div>

<div class="col-md-9">
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Edit Recommend</h1>
         

          <!-- DataTales Example -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">TOT Recommend</h6>
            </div>
            
            <div class="peer mt-2">
                        <form action="{{route('saveBeautycatRecommendBranddelete')}}" class="actionDelete" method="post">
 ...
 Delete all selected Fashion Category</button>
        				</form>
	                </div>
	                <div id="selected_brand"></div>
            <form role="form" action="{{ route('single.update',2) }}" method="post">
	          {{ csrf_field() }}
	              <div class="box-body">
				...
				<label>Select up to 5 Brands</label>	 
				<div class="table-responsive">
                <table class="table table-bordered" id="beautycategoryrecommend_table" style="width:100%">
                    <thead> 
                    <th>Id </th>                   
                    <th>Brand Name</th> 
...
function checkboxSelect(id) {
  ...
  $("#overlay").fadeIn(300);
		...
		$.ajax({
		   type:'POST',
		   url:"{{ route('saveBeautycatRecommendBrand') }}",
		   data:{
