        ...
        <div class="container-fluid">
          ...
          <!-- DataTales Example -->
            <div class="card shadow mb-4">
                ...
                <form role="form" action="{{ route('sub.user.update',$users->id) }}" method="post">
                    {{ csrf_field() }}
                    ...
                    @include('includes.messages') 
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group"> 
                                    ...
                                    <select class="form-control  form-control-sm" id="select_no_limit" name="role" multiple="multiple">
                                        ...
                                        @foreach ($roles as $key => $role)
                                            ...
                                            <?php $select=""; if(count($admin_role)>0 && !empty($admin_role)){
                                              foreach($admin_role as $admin_rol){
                                              
                                                if($admin_rol->role_id == $role->id){
                                            ...
                                            <option value="{{$role->id}}" <?php echo $select ?>>{{$role->name}}</option>
                                        @endforeach
                                ...
                                <div class="form-group"> 
                                    ...
                                    @foreach ($permissions as $key =>$permission)
                                        <?php $select=""; if(count($admin_permission)>0 && !empty($admin_permission)){
                                              foreach($admin_permission as $admin_permissio){
                                              
                                                if($admin_permissio->permission_id == $permission->id){
                                            ...
                                            <option value="{{$permission->id}}" <?php echo $select ?>>{{$permission->name}}</option>
                                        @endforeach 
                        ...
                        <div class="checkbox">
                           <label class="switch"> 
                           <input type="checkbox" name="active" @if ($users->active == 'yes')
                           checked
                           @endif value="no" >
                           <span class="slider round"></span>
							 ...
							 <div class="col-lg-offset-3 col-lg-12">
                                <div class="form-group">
                                    @if(Auth::user()->can('sub-users-edit'))
