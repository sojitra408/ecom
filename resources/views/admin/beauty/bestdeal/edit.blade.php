...
<div class="col-md-9">
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Edit Top Sell Category</h1>
         

          <!-- DataTales Example -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Head Top Sell Category</h6>
            </div>
            <div class="peer mt-2">
                        <form action="{{route('deletebeautytopsellcategory')}}" class="actionDelete" method="post">
 ...
 Delete all selected Brands</button>
        				</form>
	                </div>
	                
	  <div class="peer mt-2">
	@foreach($all_brands as $brand)
		<span class="badge badge-secondary mr-2">{{$brand->Category->name}}</span>    
	@endforeach
</div>
            ...
            <form role="form" action="#" method="post">
	          {{ csrf_field() }}
				...
				<label>Select up to 12 Category</label>	 
				<div class="table-responsive">
                <table class="table table-bordered" id="beautybesttop_sell_table" style="width:100%">
                    <thead> 
                    <th>Id</th>
                    <th>Category Name</th>
...
function checkboxSelect(id) {
       ...
       $("#overlay").fadeIn(300);
		...
		$.ajax({
		   type:'GET',
		   url:"{{ route('beautybest_top_sell_update') }}",
		   data:{
