...
class OrderExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    */
     ...
     {
        //  return orders::select(DB::raw('CONCAT(users.first_name, " ", users.last_name) as name'),'users.email','users.mobile','users.city','orders.order_id','orders.order_date','orders.order_status','orders.grand_total','orders.payment_status')->leftjoin('users','users.id','=','orders.user_id')->where('orders.order_date','>=',$this->from)->where('orders.order_date','<=', $this->to)->get();
        
        
        $orders = orders::select(DB::raw('CONCAT(users.first_name, " ", users.last_name) as name'),'users.email','users.mobile','orders.order_id','orders.order_date','orders.order_status','orders.grand_total','orders.payment_status')->leftjoin('users','users.id','=','orders.user_id')->where('orders.order_date','>=',$this->from)->Orwhere('orders.order_date','=<', $this->to)->get();
