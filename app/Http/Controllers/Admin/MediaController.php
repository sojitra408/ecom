...
$title = array('pageTitle' => 'Media display');
$web_setting=$objsetting->getSettings();
        return view("admin.media.index",$title)->with('web_setting',$web_setting);
...

    {
	 ...
	 $title = array('pageTitle' =>'Add Images');
        ...
        return view('admin.media.addimages',$title)->with('images', $images);
    ...
    public function fileUpload(Request $request){
        ...
        if($request->hasFile('file') and in_array($request->file->extension(), $extensions)){  
            ...
            $Path = 'public/images/media/' . $directory . '/' . $filename;

            $Images = new Images();
			
		 $userid=Auth('admin')->user()->id;

            $imagedata = $Images->imagedata($filename, $Path, $width, $height,$userid);
	 ...
	 public function fileUploadNew(Request $request){
        ...
        if($request->hasFile('file') and in_array($request->file->extension(), $extensions)){  
           ...
           $extension = $image->getClientOriginalExtension(); 
            ...
            $directory =date_format($time, 'Y') . '/' . date_format($time, 'm');
			 $directorySave =date_format($time, 'Y') . '/' . date_format($time, 'm');
			
			 

            // Creating the file name: random string followed by the day, random number and the hour

            $filename = time() . date_format($time, 'd') . rand(1, 9) . date_format($time, 'h') . "." . $extension;

            // This is our upload main function, storing the image in the storage that named 'public'
 $directory = public_path('/images/media/'.$directory);

 
 	 
             //$upload_success = $image->storeAs('public/images/media/'.$directory, $filename, 'public');
		  ...
		  $pathSave='public/images/media/'.$directorySave.'/'.$filename;

            $Images = new Images();

            $imagedata = $Images->imagedata($filename, $pathSave, $width, $height,$userid);
    ...
    //                $storeMediumImage = $Images->Mediumrecord($filename,$Path,$width,$height);
        ...
        // dd($upload_success);
			...
			return redirect()->back()->with('success','Uploaded Success');
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
        ...
        $namethumbnail = $thumbnailImage->save($destinationPath.'/'.'thumbnail'.time().$filename); 
           ...
           $Path =  $directory.'/'.'thumbnail'.time().$filename;
...
//        $height = $Medium_values[0]->value;
        ...
        $Path =  $directory .'/'.'medium'.time().$filename;
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
        ...
        $Path =   $directory .'/'. 'large'.time().$filename;
