...

{
...

  {
    ...
    return view("admin.blognewcategory.index",$title)->with('categories', $categories);
    }
   ...
   public function index(){
    ...
    $Categorys = BlogCategory::where('parent_id', '=', 0)->get();
        ...
        foreach ($Categorys as $Category) {
             ...
             $tree .='<li class=""><span><i class=" fas fa-folder-open"></i> '.$Category->name.'</span> '; 
              ...
              $tree .= ' <a href="'.url('admin/category?id='.$Category->id).'">Edit</a>';
				$Category->childs=BlogCategory::where('parent_id',  $Category->id)->get();
        ...
        $lastid = BlogCategory::latest()->first();

        if(!empty($lastid))

        {

          $lastid = BlogCategory::latest()->first()->id;
  ...
  $features= features::get();
        ...
        if(isset($_GET['id'])){ 
          ...
          $category = DB::table('blogcategories')->find($_GET['id']);
        } 

        if(Auth::user()->can('blog-category-list'))
    {

        return view('admin.blognewcategory.index',$title,compact('tree','lastid','data','category','features'));
    }
    return redirect()->back();
  ...
  public function childView($Category){                 
            ...
            foreach ($Category->childs as $arr) {
                ...
                $html .='<li class=""><span><i class="fas fa-minus-circle"></i></i>'.$arr->name.'</span>'; 
                  ...
                  $html .= '<a href="'.url('admin/blogcategory?id='.$arr->id).'">Edit</a>';                  
...
$fval=$request->get("fval");
    ...
    $request->validate([
      ...
      //  $Category->attribute=implode(',',$request->get("attribute"));
        ...
        return redirect()->route('admin.blogcategory')->with(['message' => 'Category Added Successfully.']);
...
$fval=$request->get("fval");
	...
	$category_id=$Category->id;
        ...
        return redirect()->route('admin.blogcategory')->with(['message' => 'Sub Category Added Successfully.']);
    ...
    public function edit(Request $request){ 
      ...
      $category = DB::table('blogcategories')->find($id);
          
          if(Auth::user()->can('blog-category-list'))
    {

      return view('admin.blognewcategory.edit',$title, compact('category'));
    }
    return redirect()->back();
   ...
   public function catdelete($id)
    {
        ...
        $Project = DB::table('blogcategories')->where('id',$id)->orwhere('parent_id',$id)->delete();
        //$Project->delete();
        return redirect()->route('admin.blogcategory')->with(['message' => 'Category Deleted Successfully.']);
    }
   ...
   public function catupdate(Request $request, $id)
    {
        ...
        return redirect()->route('admin.blogcategory')->with(['message' => 'Category Updated Successfully.']);
    }
...
if(Auth::user()->can('blog-category-list'))
    ...
    return view("admin.blogcategory.index", $title)->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter]);
    }
    return redirect()->back();
  ...
  $images = new Images;
    ...
    return view("admin.blogcategory.create",$title)->with('result', $result)->with('allimage', $allimage)->with('unit',$unit);
    }
    return redirect()->back();
...
$subcat= DB::table('blogcategories as c')->select('c.*','cd.categories_name')->join('categories_description as cd','c.categories_id','cd.categories_id')->where('c.parent_id',$request->parentID)->get();
   ...
   $objcategory = new NewCategories();
     ...
     if($request->old_slug!=$request->slug){
         ...
         do{
             ...
             $checkSlug = DB::table('blogcategories')->where('categories_slug',$currentSlug)->where('categories_id','!=',$request->id)->get();
...
$unit = $request->unit;
	...
	function topSellCategory(Request $request){
       ...
       $columns = array( 
        ...
        $totalData = BlogCategory::where('parent_id','!=',0)->get()->count();            
        ...
        $institutes = BlogCategory::where('parent_id','!=',0)->offset($start)
                         ->limit($limit)
                         ->orderBy($order,$dir)
                         ->get();
        $data = array();
        if(!empty($institutes))
        {
            ...
            {
                $check1=HomeTopCategory::where('category_id',$institute->id)->first();
                //echo $check;die;
              //  if($check>0)
                if($check1)
                {
